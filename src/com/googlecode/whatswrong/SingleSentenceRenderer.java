...
/**
 * A SingleSentenceRenderer renders an NLPInstance as a single sentence with
 * spans drawn below the tokens, and dependencies above the tokens.
 *
...
public class SingleSentenceRenderer implements NLPCanvasRenderer {
   ...
   * tokens, and dependencies above tokens.
   *
  ...
  public Dimension render(NLPInstance instance, Graphics2D graphics2D) {
    ...
    Map<Token, Bounds1D> tokenXBounds =
      tokenLayout.estimateTokenBounds(tokens, widths, graphics2D);
    ...
    dim = tokenLayout.layout(tokens, widths, graphics2D);
    height += dim.height;
    width = dim.width > width ? dim.width : width;
  ...
  /**
   * Controls whether the graph should be curved or rectangular. If curved the
   * dependencies are drawn as curves instead of rectangular lines, and spans
